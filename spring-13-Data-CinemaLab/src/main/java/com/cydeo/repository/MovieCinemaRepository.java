...
public interface MovieCinemaRepository extends JpaRepository<MovieCinema, Long> {
    ...
    // since we do not know if the given id is  there we can use Optional
    @Override
    ...
    List<MovieCinema> findAllByMovieContaining(String name);
    ...
    //Write a native query to count all movie cinemas by cinema id
     @Query(value = "SELECT count(*) movie_cinema WHERE cinema_id = ?1", nativeQuery = true)
